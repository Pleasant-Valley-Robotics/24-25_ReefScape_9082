...
public class RobotContainer {
    private double drivingScalarConstant = 1.0;
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
            ...
            drivetrain.applyRequest(() ->
                drive.withVelocityX(-joystick.getLeftY() * drivingScalarConstant * MaxSpeed) // Drive forward with negative Y (forward)
                    ...
                    .withRotationalRate(-joystick.getRightX() * drivingScalarConstant * MaxAngularRate) // Drive counterclockwise with negative X (left)
            )
        );
        ...
        if(joystick.rightBumper().getAsBoolean()){
            drivingScalarConstant = 0.5;
        }
        else{
            drivingScalarConstant = 1.0;
        }
        //Operator Joysticks
        new JoystickButton(joystick2, 1).whileTrue(new CoralEEAutoOuttake(coralEE, 1.0, 2.0));
        new JoystickButton(joystick2, 7).whileTrue(new CoralEEAutoOuttake(coralEE, -1.0, 2.0));
